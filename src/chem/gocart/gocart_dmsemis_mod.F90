...
!
  ...
  real(CHEM_KIND_R8), DIMENSION (1,1,1,1) :: tc
  real(CHEM_KIND_R8), DIMENSION (1,1,1) :: bems,airmas
  real(CHEM_KIND_R8), DIMENSION (1,1) :: emsdms
  real(CHEM_KIND_R8), dimension (1,1) :: w10m,gwet,airden,tskin,dmso
  real(CHEM_KIND_R8), dimension (1) :: dxy
  real(CHEM_KIND_R8),parameter::max_default=1.e-30
!
! number of dust bins
!
  imx=1
...
SUBROUTINE srcdms(imx, jmx, lmx, nmx, ndt1, tc,airmw, &
  ...
  REAL(CHEM_KIND_R8),    INTENT(IN)    :: tskin(imx,jmx), dmso(imx,jmx)
  ...
  REAL(CHEM_KIND_R8),    INTENT(INOUT) :: emsdms(imx,jmx)
  REAL(CHEM_KIND_R8),    INTENT(OUT)   :: bems(imx,jmx,nmx)

  INTEGER :: i,j
  REAL(CHEM_KIND_R8)    :: sst, sc, conc, w10, scco2, akw, erate, dmssrc, c
