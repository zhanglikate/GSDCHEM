...
! **************************************************************************** 
  ...
  REAL(CHEM_KIND_R8),    INTENT(IN)  :: airden(imx,jmx), delz_sfc(imx,jmx)
  REAL(CHEM_KIND_R8),    INTENT(IN)  :: hflux(imx,jmx), ts(imx,jmx)
  REAL(CHEM_KIND_R8),    INTENT(IN)  :: ustar(imx,jmx), pblz(imx,jmx)
  REAL(CHEM_KIND_R8),    INTENT(IN)  :: ps(imx,jmx)
  INTEGER, INTENT(IN)  :: ilwi(imx,jmx)
  REAL(CHEM_KIND_R8),    INTENT(IN)  :: z0(imx,jmx)
  REAL,    INTENT(IN)  :: g0
  REAL(CHEM_KIND_R8),    INTENT(OUT) :: dvel(imx,jmx), drydf(imx,jmx)
  
  REAL(CHEM_KIND_R8)    :: obk, vds, czh, rttl, frac, logmfrac, psi_h, cz, eps
  REAL(CHEM_KIND_R8)    :: vd, ra, rb, rs  
  INTEGER :: i, j, k, ldt, iolson, ii
  CHARACTER(LEN=50) :: msg
  REAL(CHEM_KIND_R8)     :: prss, tempk, tempc, xnu, ckustr, reyno, aird, diam, xm, z
  REAL(CHEM_KIND_R8)     :: frpath, speed, dg, dw, rt
  REAL(CHEM_KIND_R8)     :: rad0, rix, gfact, gfaci, rdc, rixx, rluxx, rgsx, rclx
