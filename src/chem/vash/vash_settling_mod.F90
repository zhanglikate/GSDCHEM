...
! **************************************************************************** 
  ...
  REAL(CHEM_KIND_R8)    :: tc1(imx,jmx,lmx,nmx), dt_settl(nmx), rcm(nmx), rho(nmx),addmass(lmx,nmx)
  INTEGER :: ndt_settl(nmx)
  REAL(CHEM_KIND_R8)    :: dzmin, vsettl, dtmax, pres, rhb, rwet(nmx), ratio_r(nmx)
  REAL(CHEM_KIND_R8)    :: addmassf,c_stokes, free_path, c_cun, viscosity, vd_cor, growth_fac,mass_above
  ...
  ! for OMP:
  REAL(CHEM_KIND_R8) :: rwet_priv(nmx), rho_priv(nmx),vsettl_max(nmx)
