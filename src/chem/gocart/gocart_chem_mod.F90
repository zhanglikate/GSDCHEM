...
contains
  ...
  real(CHEM_KIND_R8), DIMENSION (1,1,1) :: tmp,airden,airmas,oh,xno3,h2o2,chldms_oh,    &
                               chldms_no3,chldms_x,chpso2,chpmsa,chpso4,    &
                               chlso2_oh,chlso2_aq,cldf
  real(CHEM_KIND_R8), DIMENSION (1,1,4) :: tdry
  real(CHEM_KIND_R8), DIMENSION (1,1) :: cossza
  real, DIMENSION (1,1) :: sza,cosszax
  real(CHEM_KIND_R8), DIMENSION (1,1,1,4) :: tc,bems
...
! USE module_data_gocart
  ...
  integer :: ndt1
  real, intent(in) :: dt1
  REAL(CHEM_KIND_R8), DIMENSION(imx,jmx,lmx), INTENT(IN) :: tmp, airden, airmas
  REAL(CHEM_KIND_R8), DIMENSION(imx,jmx,lmx), INTENT(IN) :: oh, xno3, cldf
  REAL(CHEM_KIND_R8), DIMENSION(imx,jmx,lmx), INTENT(INOUT) :: h2o2
!  REAL(CHEM_KIND_R8), INTENT(IN)    :: drydf(imx,jmx,nmx)
  REAL(CHEM_KIND_R8), INTENT(INOUT) :: tc(imx,jmx,lmx,nmx)
  REAL(CHEM_KIND_R8), INTENT(INOUT) :: tdry(imx,jmx,nmx)
  real(CHEM_KIND_R8), DIMENSION (imx,jmx),INTENT(IN) :: cossza
!  REAL(CHEM_KIND_R8), DIMENSION(imx,jmx),     INTENT(INOUT) :: depso2, depso4, depmsa
  REAL(CHEM_KIND_R8), DIMENSION(imx,jmx,lmx), INTENT(INOUT) :: chldms_oh, chldms_no3
  REAL(CHEM_KIND_R8), DIMENSION(imx,jmx,lmx), INTENT(INOUT) :: chldms_x, chpso2, chpmsa
  REAL(CHEM_KIND_R8), DIMENSION(imx,jmx,lmx), INTENT(INOUT) :: chpso4, chlso2_oh, chlso2_aq

  REAL(CHEM_KIND_R8), DIMENSION(imx,jmx,lmx) :: pso2_dms, pmsa_dms, pso4_so2
...
! USE module_data_gocart_chem
  ...
  REAL(CHEM_KIND_R8), DIMENSION(imx,jmx,lmx), INTENT(IN) :: tmp, airden, airmas
  REAL(CHEM_KIND_R8), DIMENSION(imx,jmx,lmx), INTENT(IN) :: oh, xno3
  REAL(CHEM_KIND_R8), INTENT(INOUT) :: tc(imx,jmx,lmx,nmx)
  REAL(CHEM_KIND_R8), DIMENSION(imx,jmx,lmx), INTENT(INOUT) :: chldms_oh, chldms_no3
  REAL(CHEM_KIND_R8), DIMENSION(imx,jmx,lmx), INTENT(INOUT) :: chldms_x, chpso2, chpmsa
  REAL(CHEM_KIND_R8), DIMENSION(imx,jmx,lmx), INTENT(OUT)   :: pso2_dms, pmsa_dms
  real(CHEM_KIND_R8), DIMENSION (imx,jmx),INTENT(IN) :: cossza

  REAL(CHEM_KIND_R8), PARAMETER :: fx = 1.0 
  REAL(CHEM_KIND_R8), PARAMETER :: a = 0.75
  REAL(CHEM_KIND_R8), PARAMETER :: b = 0.25
  
  ! From D4: only 0.8 efficiency, also some goes to DMSO and lost. 
  ! So we assume 0.75 efficiency for DMS addtion channel to form    
  ! products.                                                       
  
  REAL(CHEM_KIND_R8), PARAMETER :: eff = 1.0
  ! -- Factor to convert AIRDEN from kgair/m3 to molecules/cm3: 
  REAL(CHEM_KIND_R8), PARAMETER :: f = 1000.0 / airmw * 6.022D23 * 1.0D-6
  INTEGER :: i, j, l
...
! USE module_data_gocart_chem
  ...
  INTEGER, INTENT(IN) ::  nmx, ndt1,imx,jmx,lmx
  REAL(CHEM_KIND_R8), DIMENSION(imx,jmx,lmx), INTENT(IN) :: tmp, airden, airmas
  REAL(CHEM_KIND_R8), DIMENSION(imx,jmx,lmx), INTENT(IN) :: cldf, oh
  REAL(CHEM_KIND_R8), DIMENSION(imx,jmx,lmx), INTENT(INOUT) :: h2o2
  real(CHEM_KIND_R8), DIMENSION (imx,jmx),INTENT(IN) :: cossza
!  REAL(CHEM_KIND_R8), INTENT(IN)    :: drydf(imx,jmx,nmx)
  REAL(CHEM_KIND_R8), INTENT(INOUT) :: tc(imx,jmx,lmx,nmx)
...
!  REAL(CHEM_KIND_R8), DIMENSION(imx,jmx),     INTENT(INOUT) :: depso2
  REAL(CHEM_KIND_R8), DIMENSION(imx,jmx,lmx), INTENT(INOUT) :: chpso4, chlso2_oh, chlso2_aq
  REAL(CHEM_KIND_R8), INTENT(IN)  :: pso2_dms(imx,jmx,lmx)
  REAL(CHEM_KIND_R8), INTENT(OUT) :: pso4_so2(imx,jmx,lmx)

  REAL(CHEM_KIND_R8) ::  k0, kk, m, l1, l2, ld
  ! Factor to convert AIRDEN from kgair/m3 to molecules/cm3: 
  REAL(CHEM_KIND_R8), PARAMETER :: f  = 1000. / airmw * 6.022D23 * 1.0D-6
  REAL(CHEM_KIND_R8), PARAMETER :: ki = 1.5D-12
  INTEGER :: i, j, l
  REAL(CHEM_KIND_R8) :: so20, tk, f1, rk1, rk2, rk, rkt, so2_cd, fc, so2
...
! USE module_data_gocart_chem
  ...
  INTEGER, INTENT(IN) :: nmx, ndt1,imx,jmx,lmx
  REAL(CHEM_KIND_R8), DIMENSION(imx,jmx,lmx), INTENT(IN) :: airmas
!  REAL(CHEM_KIND_R8), INTENT(IN)    :: drydf(imx,jmx,nmx)
  REAL(CHEM_KIND_R8), INTENT(INOUT) :: tc(imx,jmx,lmx,nmx)
  REAL(CHEM_KIND_R8), INTENT(INOUT) :: tdry(imx,jmx,nmx)

!  REAL(CHEM_KIND_R8), DIMENSION(imx,jmx), INTENT(INOUT) :: depso4
  REAL(CHEM_KIND_R8), INTENT(IN) :: pso4_so2(imx,jmx,lmx)
  real(CHEM_KIND_R8), DIMENSION (imx,jmx),INTENT(IN) :: cossza

  INTEGER :: i, j, l
  REAL(CHEM_KIND_R8) :: so40, rk, rkt, so4 
...
!  REAL, DIMENSION(imx,jmx), INTENT(INOUT) :: depmsa
  REAL(CHEM_KIND_R8), INTENT(IN) :: pmsa_dms(imx,jmx,lmx)

  REAL(CHEM_KIND_R8) :: msa0, msa, rk, rkt
  INTEGER :: i, j, l
