...
! !INPUT PARAMETERS:
   ...
   real,    intent(in)           :: w10m        ! 10-m wind speed [m s-1]
   real,    intent(in)           :: ustar       ! friction velocity [m s-1]
   integer, intent(in)           :: method      ! Algorithm to use

! !OUTPUT PARAMETERS:

   real,    intent(inout)        :: memissions      ! Mass Emissions Flux [kg m-2 s-1]
   real,    intent(inout)        :: nemissions      ! Number Emissions Flux [# m-2 s-1]
   integer, intent(out)          :: rc              ! Error return code:
                                                    !  0 - all is well
                                                    !  1 - 
! !Local Variables
   integer       :: ir
   real          :: w                               ! Intermediary wind speed [m s-1]
