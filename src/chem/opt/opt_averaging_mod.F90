...
! 7/21/09  SAM variables needed to convert GOCART sectional dust and seasalt to MOZAIC sections
   ...
   real(CHEM_KIND_R8) :: dlogoc, dhigoc
   integer istop,pdust
   integer, save :: kcall
   data  kcall / 0 /
...
!       integer,parameter :: nspint = 4 ! Num of spectral for FAST-J
        ...
        integer n
        real(CHEM_KIND_R8) thesize    ! 2 pi radpart / waveleng = size parameter
        real(CHEM_KIND_R8) qext(nsiz) ! array of extinction efficiencies
        real(CHEM_KIND_R8) qsca(nsiz) ! array of scattering efficiencies
        real(CHEM_KIND_R8) gqsc(nsiz) ! array of asymmetry factor * scattering efficiency
...
!
      ...
      real(CHEM_KIND_R8)      mim, mimsav, mre, mm, np1dn
...
! /*---------------------------------------------------------------*/
      REAL(CHEM_KIND_R8) VLAMBc,RGcmin,RGcmax,RGc,SIGMAGc,SHELRc,SHELIc
      REAL(CHEM_KIND_R8) RINc,CORERc,COREIc
      INTEGER*4 NRGFLAGc,NANG
      REAL(CHEM_KIND_R8) QEXTc,QSCATc,QBACKc,EXTc,SCATc,BACKc,GSCAc
      REAL(CHEM_KIND_R8) ANGLESc(200),S1R(200),S1C(200),S2R(200),S2C(200)
...
!     /*--------------------------------------------------------*/
      ...
      REAL(CHEM_KIND_R8)     RO, RFR, RFI, THETD(IT), QEXT, QSCAT, CTBRQS, &
...
!     /*--------------------------------------------------------*/
      ...
      REAL(CHEM_KIND_R8)     T(5), TA(4), TB(2), TC(2), TD(2), TE(2), X, &
...
!     /*--------------------------------------------------------*/
      ...
      IMPLICIT REAL(CHEM_KIND_R8) (A-H, O-Z)
...
!     /*--------------------------------------------------------*/
      ...
      REAL(CHEM_KIND_R8) QEXTc,QSCATc,QBACKc,EXTc,SCATc,BACKc,GSCAc
      REAL(CHEM_KIND_R8) ANGLESc(*),S1R(*),S1C(*),S2R(*),S2C(*)
      REAL(CHEM_KIND_R8) S11N,S11(*),S12(*),S33(*),S34(*),SPOL(*),SP(*)
...
!     /*--------------------------------------------------------*/
      ...
      REAL(CHEM_KIND_R8)  THETA(MXNANG), ELTRMX(4,MXNANG,2), PII(3,MXNANG), &
